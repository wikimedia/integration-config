...
class TestZuulScheduler(unittest.TestCase):
    ...
    def assertProjectHasI18nChecker(self, name, definition, pipeline):
        ...
        self.assertTrue(
            any([job for job in definition
                 if 'mediawiki-i18n-check' == job]),
            'Project %s pipeline %s must have job mediawiki-i18n-check'
            % (name, pipeline)
            )
    ...
    def test_gated_extensions_lists_are_in_sync(self):
        ...
        gated_in_zuul = set([
            ...
            and 'wmf-quibble-vendor-mysql-php74'
    ...
    def test_postmerge_gerrit_review_command(self):
        ...
        self.assertEquals(
            ('gerrit review '
             ...
             '--message "hello world" '
             '--tag autogenerated:ci-postmerge 12345,42'
             ),
            cmd,
            'gerrit review command does not match Gerrit 2.13 expectation: %s')
        ...
        for pipeline in self.getPipelines():
            ...
            for action in self.sched._reporter_actions.values():
                ...
                for reporter in [r for r in getattr(pipeline, action, [])
                    ...
                    try:
                        self.assertIn(
                            ' --tag %s ' % expected_tag, cmd,
                            'Pipeline %s action %s must use '
                            'tag: %s' % (
                                pipeline.name, action, expected_tag))
    ...
    def test_mwcore_switch_to_quibble(self):
        ...
        expected_gate = {
            ...
            'mediawiki-quibble-vendor-mysql-php74': True,
            'mediawiki-quibble-vendor-mysql-php80': True,
